...
def get_total_sum(order_id):
    query = "SELECT SUM(I.quantity * B.price) FROM order_items I, (SELECT DISTINCT ON (product_id) product_id, price FROM prices WHERE created_at <= (SELECT COALESCE (sent_at, NOW()) FROM orders WHERE id=:order_id) ORDER BY product_id, created_at DESC) B WHERE I.product_id=B.product_id AND I.order_id=:order_id"
...
def send_order(order_id):
    ...
    for product_id in product_ids:
        ...
        db.session.execute(query, {"quantity": quantity_in_stock - quantity_in_order, "product_id": product_id[0]})
...
def get_order_information(order_id):
    query = "SELECT A.id, A.name, B.price, I.quantity, B.price*I.quantity AS subtotal FROM products A, orders O, order_items I, (SELECT DISTINCT ON (product_id) product_id, price FROM prices WHERE created_at <= (SELECT sent_at FROM orders WHERE id=:order_id) ORDER BY product_id, created_at DESC) B WHERE A.id=B.product_id AND O.id=:order_id AND O.id=I.order_id AND A.id=I.product_id ORDER BY subtotal DESC"
